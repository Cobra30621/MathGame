...
// 一般關卡資訊
public class NoBossStageData : IStageData
{

    // ================= 初始設定方法 ===================
	public NoBossStageData(float CoolDown , string name, int[] primes, int[] composites, int[] plusNums, int[] bossNums):base(CoolDown , name, primes, composites, plusNums, bossNums)
	{
		// 設定冷卻時間
    ...
    public override void WaitNoBallProcess(){
        ...
        if (ballCount == 0)
            SetGameProcess(GameProcess.GameEnd); // 遊戲結束
